  ...
  return (
    <>
      <div className='pt-36'>
        ...
        <Sidebar className={`wrap absolute shadow-xl top-0 left-0 bg-white h-full w-3/4 md:w-1/4 duration-500 transition-all ${menu ? 'ml-0' : '-ml-[500px]'}`} onShippingAddress={()=>{handleOpenModal(); close()}} onAccount={()=>{setShow('my-account'); close()}} onProduct={()=>{setShow('my-product'); close()}} onSelling={()=>{setShow('product-selling'); close()}} onOrder={()=>{setShow('my-order'); close(); handleCloseModal()}} onAccountCustomer={()=>{setShow('my-account-customer'); close(); handleCloseModal()}} clickEdit={()=>{setEdit(true); close()}} />
        <button onClick={()=>menu ? close() : open()} className={`absolute hover:text-red-600 top-28 md:top-40 md:left-7 duration-500 ${menu === true ? 'left-[310px] rotate-180 md:left-[420px]' : 'left-5 rotate-0'}`}>
